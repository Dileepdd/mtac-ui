// Minimal stroke-based icon set. 16px default, currentColor.
// Geometric, hand-trimmed. No emoji.

const Icon = ({ d, s, size = 16, stroke = 1.5, fill = "none", style }) => (
...
const I = {
  // Brand glyph: stacked squares rotated
  logo: (props) => (
    <svg width={props?.size || 22} height={props?.size || 22} viewBox="0 0 24 24" aria-hidden="true">
...
const PriorityBars = ({ level, size = 12 }) => {
  const map = { urgent: 4, high: 3, med: 2, low: 1, none: 0 };
  const n = map[level] ?? 0;
  const color = { urgent: "var(--priority-urgent)", high: "var(--priority-high)", med: "var(--priority-med)", low: "var(--priority-low)", none: "var(--text-4)" }[level];
  return (
    <svg width={size} height={size} viewBox="0 0 14 14" aria-hidden="true" style={{ color }}>
      {[0, 1, 2, 3].map(i => (
        <rect key={i} x={1 + i * 3.3} y={10 - i * 2.5} width="2.3" height={3 + i * 2.5} rx="0.6"
...
Object.assign(window, { I, Icon, StatusDot, PriorityBars });
